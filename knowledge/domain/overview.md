...
Netok и Kotic — личные продукты, не клиентские. У обоих в `content` стоит
заглушка «Case study in progress», при этом страницы `/portfolio/netok/` и
`/portfolio/kotic/` генерируются и доступны по прямой ссылке, хотя ссылок на них
нет нигде.
